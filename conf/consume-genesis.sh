...
if [ -z "$HAS_VALIDATOR" ]; then
  if [ -f "$AXELAR_MNEMONIC_PATH" ]; then
    ...
    (
      echo "${KEYRING_PASSWORD}"
      echo "$KEYRING_PASSWORD"
    ) | axelard keys add validator >"/home/axelard/shared/validator.txt" 2>&1
  fi
fi
...
if [ ! -f "/home/axelard/.axelar/config/genesis.json" ]; then
  ...
  cp "/home/axelard/conf/genesis.json" "/home/axelard/.axelar/config/genesis.json"
fi

cp "/home/axelard/conf/seeds.toml" "/home/axelard/.axelar/config/seeds.toml"
